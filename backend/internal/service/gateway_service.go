...
func saveUsageLogDetailBestEffort(
	ctx context.Context,
	svc *UsageLogDetailService,
	enabled bool,
	usageLog *UsageLog,
	input UsageLogDetailSaveInput,
	logKey string,
) {
	if svc == nil || !enabled || usageLog == nil || IsUsageRecordSyncFallback(ctx) {
		return
	}
	if input.UsageLogID <= 0 {
	...
	if strings.TrimSpace(input.RequestID) == "" || input.APIKeyID <= 0 {
		return
	}
	if err := svc.Save(ctx, input); err != nil {
		logger.LegacyPrintf(logKey, "Save usage log detail failed: %v", err)
	}
}
...
func (s *GatewayService) RecordUsage(ctx context.Context, input *RecordUsageInput) error {
	...
	if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple {
		if writeUsageLogBestEffort(ctx, s.usageLogRepo, usageLog, "service.gateway") {
			saveUsageLogDetailBestEffort(ctx, s.usageLogDetailService, s.usageDetailCapture.Enabled, usageLog, detailInput, "service.gateway")
		}
