...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StoreDisabledStrictAffinityPreflightPingFailReconnectsFunctionCallOutputWithPreviousResponseID(t *testing.T) {
	gin.SetMode(gin.TestMode)
	prevPreflightPingIdle := openAIWSIngressPreflightPingIdle
	openAIWSIngressPreflightPingIdle = 0
	defer func() {
	...
	firstConn := &openAIWSPreflightFailConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_ping_fco_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	secondConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_turn_ping_fco_2","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		},
	}
	dialer := &openAIWSQueueDialer{
		conns: []openAIWSClientConn{firstConn, secondConn},
	...
	account := &Account{
		ID:          122,
		Name:        "openai-ingress-preflight-ping-function-call-output",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"input":[{"type":"input_text","text":"hello"}]}`)
	firstTurn := readMessage()
	require.Equal(t, "resp_turn_ping_fco_1", gjson.GetBytes(firstTurn, "response.id").String())

	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"previous_response_id":"resp_turn_ping_fco_1","input":[{"type":"function_call_output","call_id":"call_turn_ping_fco_1","output":"ok"}]}`)
	secondTurn := readMessage()
	require.Equal(t, "resp_turn_ping_fco_2", gjson.GetBytes(secondTurn, "response.id").String())
	...
	case serverErr := <-serverErrCh:
		require.NoError(t, serverErr)
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket function_call_output preflight ping 自动恢复后结束超时")
	}

	require.Equal(t, 2, dialer.DialCount(), "function_call_output 严格亲和 preflight ping 失败后应换连继续续链")
	require.Equal(t, 1, firstConn.WriteCount(), "preflight ping 失败后不应继续在旧连接写第二轮")
	require.GreaterOrEqual(t, firstConn.PingCount(), 1, "第二轮前应执行 preflight ping")
	secondConn.mu.Lock()
	secondWrites := append([]map[string]any(nil), secondConn.writes...)
	...
	require.Len(t, secondWrites, 1)
	secondWrite := requestToJSONString(secondWrites[0])
	require.Equal(t, "resp_turn_ping_fco_1", gjson.Get(secondWrite, "previous_response_id").String(), "function_call_output 重连时应保留 previous_response_id")
	require.Equal(t, 1, len(gjson.Get(secondWrite, "input").Array()), "function_call_output 重连时不应被改写成 full create")
	require.Equal(t, "function_call_output", gjson.Get(secondWrite, "input.0.type").String())
	require.Equal(t, "call_turn_ping_fco_1", gjson.Get(secondWrite, "input.0.call_id").String())
	require.Equal(t, "ok", gjson.Get(secondWrite, "input.0.output").String())
}
