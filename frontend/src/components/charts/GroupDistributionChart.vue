...
const displayGroupStats = computed(() => {
  ...
  return [...props.groupStats].sort((a, b) => getGroupMetricValue(b) - getGroupMetricValue(a))
})
...
const chartData = computed(() => {
  ...
  return {
    ...
    datasets: [
      {
        data: displayGroupStats.value.map((g) => getGroupMetricValue(g)),
...
const getGroupMetricValue = (group: GroupStat): number => (
  props.metric === 'actual_cost'
    ? toFiniteNumber(group.actual_cost)
    : toFiniteNumber(group.total_tokens)
)
